...
import path from 'path'; // Route imports
...
import logger from "./middleware/logger"; // Define logger level

Logger.setLevel(config.logLevel); // Initialize server
...
v1router.use(employees.routes()); // Declare root routes

router.use(index.routes());
router.use('/v1', v1router.routes()); // middleware for file uploading

koa.use(multer({
  storage: fileStorage
}).single('file')); //Set static folder for uploading images

koa.use(serve('./uploads')); // Dev logging middleware

koa.use(logger); // Middleware
...
koa.use(router.routes()); // Listen to port
