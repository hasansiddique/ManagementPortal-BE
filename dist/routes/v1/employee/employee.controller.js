...
import config from "../../../config"; // @desc      create employee
...
export const addEmp = asyncHandler(async ctx => {
  ...
  const {
    name,
    email
  } = ctx.request.body; // if user add file then execute below condition otherwise go next

  if (ctx.request.file) {
    const file = ctx.request.file; // Make sure the image is a photo

    ctx.assert(file.mimetype.startsWith('image'), 400, 'Please upload an image file'); // Check fileSize

    ctx.assert(file.size <= config.uploadConfig.MAX_FILE_UPLOAD, 400, 'Please upload an image less than 2MB'); // Add file to request body
  ...
  ctx.assert(!employee, 400, 'user with same credentials already exists'); // make employee password on the fly

  const password = email.concat('.', 'warlords', 123); // creates employee account

  const empAccount = await UserModel.create({
    username: name,
    email,
    password
  }); // add employee to request body so that it relates to its specific account

  ctx.request.body.user = empAccount.id; // save employee data
...
export const uptEmp = asyncHandler(async ctx => {
  ...
  const employee = await employeeModel.findById(ctx.params.id);
  ctx.assert(employee, 404, 'Employee not found with this id'); // if user add file then execute below condition otherwise go next
  ...
  await UserModel.findByIdAndUpdate(empData.user, {
    email
  }, {
    new: true,
    runValidators: true
