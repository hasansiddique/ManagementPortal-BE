...
import config from "../../../config"; // @desc      Register user
...
export const registerUser = asyncHandler(async ctx => {
  ...
  user = await UserModel.find(); // if (user.length > 1) {
...
export const loginUser = asyncHandler(async ctx => {
  const {
    email,
    password
  } = ctx.request.body; // Validate emil & password
  ...
  const userById = await UserModel.findById(user.id); // Create token

  const token = user.getJwtToken();
  const refreshToken = user.getSignedRefreshJwtToken();
  const test2 = await UserModel.findByIdAndUpdate(user.id, {
    refreshToken
  }, {
    new: true,
    runValidators: true
...
export const logOut = asyncHandler(async ctx => {
  ...
  ctx.body = {
    success: true,
    status: 'User successfully logout'
  };
